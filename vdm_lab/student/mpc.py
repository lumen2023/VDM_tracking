...
def nearest_horizon_reference(state, reference, config):
    """Build a continuous-yaw [x, y, speed, yaw] reference over the horizon."""
    path = reference.path
...
def update_kinematic_array(z, acceleration, steer, config):
    """Advance the kinematic bicycle state used to initialize MPC iterations."""
    dt = config.sim.dt
...
def predict_motion(z0, acceleration, steer, z_ref, config):
    """Roll out a nominal trajectory for sequential linearization."""
    z_bar = np.zeros_like(z_ref)
    z_bar[:, 0] = z0
    predicted_state = np.array(z0, dtype=float)
    for i in range(config.controller.mpc_horizon):
        predicted_state = update_kinematic_array(
            predicted_state, acceleration[i], steer[i], config
        )
        z_bar[:, i + 1] = predicted_state
    return z_bar
...
def solve_linear_mpc(z_ref, z_bar, z0, previous_steer, config, initial_steer=None):
    try:
        import cvxpy as cp
    ...
    cost = 0.0
    constraints = [z[:, 0] == z0]
    initial_steer = previous_steer[0] if initial_steer is None else initial_steer
    max_steer_step = vehicle.max_steer_rate * config.sim.dt
    ...
    constraints += [
        ...
        cp.abs(u[1, 0] - initial_steer) <= max_steer_step,
    ]
...
def iterative_mpc(z_ref, z0, previous_control, config):
    """Alternate nominal prediction and QP solution until control converges."""
    horizon = config.controller.mpc_horizon
    ...
    for _ in range(config.controller.mpc_iter_max):
        ...
        acceleration, steer, prediction = solve_linear_mpc(
            z_ref, z_bar, z0, steer, config, initial_steer=previous_control.steer
        )
        difference = max(
            np.max(np.abs(acceleration - old_acceleration)),
            np.max(np.abs(steer - old_steer)),
        )
        if difference < config.controller.mpc_du_threshold:
            break
    max_steer_step = config.vehicle.max_steer_rate * config.sim.dt
    acceleration[0] = clamp(
        float(acceleration[0]), -config.vehicle.max_decel, config.vehicle.max_accel
    )
    steer[0] = clamp(
        float(steer[0]),
        previous_control.steer - max_steer_step,
        previous_control.steer + max_steer_step,
    )
...
def control(state, reference, previous_control, config):
    ...
    acceleration, steer, prediction = iterative_mpc(
