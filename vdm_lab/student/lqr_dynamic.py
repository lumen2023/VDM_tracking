...
def dynamic_feedforward(speed, curvature, K, config):
    ...
    kv = (
        vehicle.lr * mass / (2.0 * vehicle.cf * wheelbase)
        - vehicle.lf * mass / (2.0 * vehicle.cr * wheelbase)
    )
    yaw_steady_error = (
        vehicle.lr * curvature
        - vehicle.lf * mass * v * v * curvature / (2.0 * vehicle.cr * wheelbase)
    )
...
def control(state, reference, previous_control, config):
    ...
    K = solve_lqr(
        A, B,
        controller.lqr_q, controller.lqr_r,
        controller.lqr_eps, controller.lqr_max_iter,
    )
    ...
    goal_distance = math.hypot(
        state.x - reference.path.x[-1],
        state.y - reference.path.y[-1],
    )
    acceleration = speed_pid(
        reference.target_speed, state.v, goal_distance, controller, vehicle,
    )
