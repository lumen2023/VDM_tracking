...
> **位置说明 / Location note.** 本文件原为仓库根目录的 `README.md`（中英双语部署与运行手册）。
> 合并两位同学的工程时，根目录 README 被替换为实验报告，本手册移至 `docs/SETUP.md` 原样保留。
> 文中所有相对路径均以**仓库根目录**为基准；链接已按新位置改写为 `../`，以便在 `docs/` 下仍可打开。
>
> This manual used to be the repository-root `README.md` (bilingual deployment and run manual).
> When the two members' projects were merged, the root README became the project report; this
> manual was preserved here unchanged. All relative paths below are relative to the
> **repository root**; its links were rewritten with `../` so they still resolve from `docs/`.
>
