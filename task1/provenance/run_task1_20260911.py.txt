...
def run_case(case, output_root, speed_mode):
    ...
    write_json(destination / "run_info.json", {
        ...
        "command_equivalent": (f"python run_experiment.py --algo {case.algo} --version solution --route {case.route} --speed-mode {speed_mode} --save-log --save-fig"
                               if case.max_steer_deg == 35 else "Use scripts/run_task1.py with the saved config.json; original CLI has no max_steer override."),
    })
    if records:
...
def source_hashes():
    ...
    files += [ROOT / "run_experiment.py", ROOT / "requirements.lock.txt", Path(__file__).resolve()]
    return {str(path.relative_to(ROOT)): hashlib.sha256(path.read_bytes()).hexdigest() for path in files}


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output", type=Path, default=ROOT / "outputs" / "task1" / datetime.now().strftime("%Y%m%d_%H%M%S"))
