...
from task1.scripts.run_task1 import Case, ObservedController, extended_metrics, make_cases, make_config, source_hashes
from task1.scripts.analyze_task1 import export_table, load_runs, validate_run
from task1.scripts.package_task1 import LOCAL_ONLY, package_files
import numpy as np
...
DELIVERED = TASK_ROOT / "data" / "20260911_task1"
...
class LayoutTests(unittest.TestCase):
    def test_new_manifests_use_consolidated_paths(self):
        hashes = source_hashes()
        self.assertIn("task1/requirements.lock.txt", hashes)
        self.assertIn("task1/scripts/run_task1.py", hashes)
        self.assertNotIn("requirements.lock.txt", hashes)
        self.assertNotIn("scripts/run_task1.py", hashes)

    def test_package_has_one_top_level_directory(self):
        files = package_files()
        self.assertGreater(len(files), 100)
        self.assertEqual({path.relative_to(ROOT).parts[0] for path in files}, {"task1"})

    def test_package_excludes_local_outputs_environments_and_archives(self):
        for path in package_files():
            with self.subTest(path=path):
                self.assertFalse(set(path.relative_to(TASK_ROOT).parts) & LOCAL_ONLY)
                self.assertNotIn(path.suffix, {".zip", ".pyc", ".pyo"})

    def test_document_links_resolve_after_relocation(self):
...
class DeliveredDataTests(unittest.TestCase):
    ...
    def test_simulation_source_matches_provenance(self):
        relocated = json.loads((TASK_ROOT / "provenance" / "path_map.json").read_text())
        for relative, expected in self.manifest["source_sha256"].items():
            with self.subTest(file=relative):
                actual = hashlib.sha256((ROOT / relocated.get(relative, relative)).read_bytes()).hexdigest()
